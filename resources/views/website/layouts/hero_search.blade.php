...
function closeHotelResults() {
    const modal = document.getElementById('hotel-results-modal');
    if (modal) {
        modal.classList.remove('active');
    }
}

// Add event listener to hotel search button
document.addEventListener('DOMContentLoaded', function() {
    const hotelSearchBtn = document.querySelector('#hotel-content .search-btn');
    if (hotelSearchBtn) {
        hotelSearchBtn.addEventListener('click', searchHotels);
    }
});

// Add this to your existing formatDateForAPI function to ensure it works for hotel dates
function formatDateForAPI(day, monthYear) {
    ...
    const year = monthYear.includes("'") ? 
        ...
        '2025'; // Default to 2025 if format is different
