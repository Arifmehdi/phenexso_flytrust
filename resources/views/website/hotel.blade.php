...
@section('meta')
<meta name="description" content="Find the best hotels at competitive prices. Search and book your perfect stay.">
          ...
          <div class="filter-bar d-flex align-items-center justify-content-between">
            <div class="filter-bar-filter d-flex flex-wrap align-items-center">
              ...
              <div class="filter-option">
                <div class="dropdown dropdown-contain">
                  <a class="dropdown-toggle dropdown-btn dropdown--btn" href="#" role="button" data-bs-toggle="dropdown"
            ...
            <div class="select-contain select2-container-wrapper">
              <select class="select-contain-select" id="sort_by" onchange="applyFilters()">
                ...
                <option value="star_desc" {{ request('sort') == 'star_desc' ? 'selected' : '' }}>Star Rating: High to
                  Low</option>
                <option value="star_asc" {{ request('sort') == 'star_asc' ? 'selected' : '' }}>Star Rating: Low to High
                </option>
              </select>
            </div>
          </div>
            ...
            <div class="sidebar-widget-item">
              <div class="contact-form-action">
                <form action="{{ route('hotel') }}" method="GET" id="filterForm">
                  ...
                  <div class="sidebar-widget">
                    <h3 class="title stroke-shape">Destination</h3>
                  ...
                  <div class="input-box">
                    ...
                    <div class="form-group">
                      ...
                      <input class="form-control" type="text" name="destination" placeholder="Destination, hotel name"
                  ...
                  <div class="sidebar-widget">
                    ...
                    <div class="sidebar-category">
                      <div class="custom-radio">
                        <input type="radio" name="sort_radio" id="sort_def" value="default"
                          {{ request('sort', 'default') == 'default' ? 'checked' : '' }}
                          onchange="syncSortSidebar(this.value)">
                        <label for="sort_def" class="ms-2">Default</label>
                      </div>
                      <div class="custom-radio">
                        <input type="radio" name="sort_radio" id="sort_pa" value="price_asc"
                          {{ request('sort') == 'price_asc' ? 'checked' : '' }} onchange="syncSortSidebar(this.value)">
                        <label for="sort_pa" class="ms-2">Price: Low to High</label>
                      </div>
                      <div class="custom-radio">
                        <input type="radio" name="sort_radio" id="sort_pd" value="price_desc"
                          {{ request('sort') == 'price_desc' ? 'checked' : '' }} onchange="syncSortSidebar(this.value)">
                        <label for="sort_pd" class="ms-2">Price: High to Low</label>
                      </div>
                      <div class="custom-radio">
                        <input type="radio" name="sort_radio" id="sort_sd" value="star_desc"
                          {{ request('sort') == 'star_desc' ? 'checked' : '' }} onchange="syncSortSidebar(this.value)">
                        <label for="sort_sd" class="ms-2">Star Rating: High to Low</label>
                      </div>
                      <div class="custom-radio">
                        <input type="radio" name="sort_radio" id="sort_sa" value="star_asc"
                          {{ request('sort') == 'star_asc' ? 'checked' : '' }} onchange="syncSortSidebar(this.value)">
                  ...
                  <div class="sidebar-widget">
                    ...
                    <div class="sidebar-review">
                      ...
                      <div class="custom-checkbox">
                        <input type="checkbox" class="form-check-input" id="star{{ $star }}" name="stars[]"
                          value="{{ $star }}" {{ in_array($star, request('stars', [])) ? 'checked' : '' }}
                  ...
                  <div class="sidebar-widget">
                    ...
                    <div class="sidebar-category">
                      <div class="custom-checkbox">
                        <input type="checkbox" class="form-check-input" id="r_exc" name="review_score[]" value="9"
                          {{ in_array(9, request('review_score', [])) ? 'checked' : '' }} onchange="this.form.submit()">
                        <label for="r_exc">Excellent (9+)</label>
                      </div>
                      <div class="custom-checkbox">
                        <input type="checkbox" class="form-check-input" id="r_vg" name="review_score[]" value="8"
                          {{ in_array(8, request('review_score', [])) ? 'checked' : '' }} onchange="this.form.submit()">
                        <label for="r_vg">Very Good (8+)</label>
                      </div>
                      <div class="custom-checkbox">
                        <input type="checkbox" class="form-check-input" id="r_g" name="review_score[]" value="7"
                          {{ in_array(7, request('review_score', [])) ? 'checked' : '' }} onchange="this.form.submit()">
      ...
      <div class="col-lg-8">
        ...
        <div class="card-item card-item-list">
          ...
          <div class="card-body">
            ...
            @endphp
            <p class="card-meta hotel-address" data-lat="{{ $hotelLatitude }}" data-lng="{{ $hotelLongitude }}"
            ...
            <p class="card-meta" style="font-size: 12px; color: #6b7280; margin-top: 4px;">
              ...
              <a href="https://www.google.com/maps/search/?api=1&query={{ $hotelLatitude }},{{ $hotelLongitude }}"
                target="_blank" class="btn-text text-primary ms-2" style="font-size: 12px; text-decoration: none;">
            ...
            <div class="card-price d-flex align-items-center justify-content-between">
              ...
              <div class="d-flex align-items-center">
                <button type="button" class="btn-text me-3"
                  onclick="showHotelDetail({{ json_encode($hotel->agoda_data) }}, '{{ addslashes($hotel->title) }}', '{{ addslashes($displayAddress) }}', {{ $hotelLatitude ?: 'null' }}, {{ $hotelLongitude ?: 'null' }})">More
                  Details<i class="la la-angle-right"></i></button>
                <a href="{{ $hotel->agoda_data['landingURL'] }}" target="_blank" class="theme-btn theme-btn-small">Book
                  on Agoda</a>
              </div>
              @else
        ...
        <div class="alert alert-warning">
          No hotels found in <strong>{{ request('country') }}</strong>. Try selecting a different country or removing
          the filter.
        </div>
        @else
  ...
  async function getLocationFromCoordinates(lat, lon) {
    ...
    try {
      ...
      if (data && data.address) {
        const city = data.address.city || data.address.town || data.address.village || data.address.suburb || data
          .address.county || '';
    ...
    if ((!displayAddress || displayAddress.startsWith('Location:')) && latitude && longitude) {
      try {
        ...
        if (location && location.fullAddress) {
          ...
          displayAddress =
            `${location.name}${location.area ? ', ' + location.area : ''}${location.country ? ', ' + location.country : ''}`;
        }
      } catch (err) {
  ...
  $(document).ready(function() {
    ...
    function initPriceSliders() {
      ...
      if ($("#slider-range2").length && $.fn.slider) {
        ...
        $("#amount2").val("$" + $("#slider-range2").slider("values", 0) + " - $" + $("#slider-range2").slider(
          "values", 1));
      }
    }
