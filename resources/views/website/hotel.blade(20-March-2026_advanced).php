...
function applyFilters() {
    ...
    // Sync price filter values from slider to form
    var minPrice = document.getElementById('min_price');
    var maxPrice = document.getElementById('max_price');
    if (minPrice && maxPrice) {
        document.getElementById('form_min_price').value = minPrice.value;
        document.getElementById('form_max_price').value = maxPrice.value;
    }
...
// Reverse geocode coordinates to a human-friendly address (via backend API)
async function getLocationFromCoordinates(lat, lon) {
    ...
    try {
        ...
        if (getLocationFromCoordinates.lastCall) {
            const timeSinceLastCall = Date.now() - getLocationFromCoordinates.lastCall;
            if (timeSinceLastCall < 1000) {
                await new Promise(resolve => setTimeout(resolve, 1000 - timeSinceLastCall));
            }
        }
        getLocationFromCoordinates.lastCall = Date.now();
        ...
        if (data.success && data.address) {
            ...
            const city = addressParts[0] || 'Unknown location';
            const country = addressParts[addressParts.length - 1] || '';
            const area = addressParts.length > 2 ? addressParts[1] : '';
        ...
        return null; // Will fall back to coordinates
    }
}
...
async function showHotelDetail(data, title, address, latitude, longitude) {
    ...
    if ((!displayAddress || displayAddress.startsWith('Location:')) && latitude && longitude) {
        try {
            ...
            // If geocoding fails, keep the original address or coordinates
        } catch (err) {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        setTimeout(resolveHotelAddresses, 500);
    });
} else {
    // Add a small delay to ensure all elements are rendered
    setTimeout(resolveHotelAddresses, 500);
...
$(document).ready(function() {
  ...
  function initPriceSliders() {
      ...
      // Initialize first slider (#slider-range in dropdown)
          ...
          try {
              $("#slider-range").slider({
                  ...
                  slide: function(event, ui) {
                      ...
                      // Sync with second slider
                      if ($("#slider-range2").length) {
                          $("#slider-range2").slider("values", [ui.values[0], ui.values[1]]);
                          $("#amount2").val("$" + ui.values[0] + " - $" + ui.values[1]);
                      }
                  }
              });
              $("#amount").val("$" + $("#slider-range").slider("values", 0) + " - $" + $("#slider-range").slider("values", 1));
              console.log('Slider #slider-range initialized');
          } catch (e) {
              console.error('Error initializing slider-range:', e);
          }
      }
      
      // Initialize second slider (#slider-range2 in sidebar)
      if ($("#slider-range2").length && typeof $.fn.slider === 'function') {
          try {
              ...
              console.log('Slider #slider-range2 initialized');
          } catch (e) {
              console.error('Error initializing slider-range2:', e);
          }
      }
  }
  ...
  // Also try to initialize on document ready as backup
  $(document).ready(function() {
      setTimeout(function() {
